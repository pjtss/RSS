...
export function UsTurnoverRatioScanner() {
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.container}>
      <div className={styles.headerArea}><div className={styles.titleGroup}><p className={styles.kicker}>LIVE QUANT TERMINAL</p><h1>시총 대비 거래대금 스캐너</h1><p className={styles.subtitle}>AMS·NAS·NYS 상승률 TOP 100 중 조건 충족 종목</p></div></div>
      {error && <div className={styles.errorAlert}>ERROR: {error}</div>}
      {loading ? <div className={styles.loaderArea}><p>실시간 데이터를 분석하는 중...</p></div> : items.length === 0 ? <div className={styles.emptyArea}><p>조건에 맞는 종목이 없습니다.</p></div> : <div className={styles.tableWrapper}><table className={styles.table}><thead><tr><th>티커</th><th>등락률</th><th>시총 대비</th><th>1분 변화</th><th>시가총액</th><th>거래대금</th></tr></thead><tbody>{items.map((item) => { const oneMinuteUp = item.trend?.oneMinuteIncrease != null && item.trend.oneMinuteIncrease > 0; return <tr key={`${item.market}-${item.code}`} className={oneMinuteUp ? styles.oneMinuteUp : undefined}><td>{item.code}</td><td>{item.changeRate}</td><td>{item.turnoverRatio.toFixed(2)}%</td><td className={oneMinuteUp ? styles.oneMinuteValue : undefined}>{item.trend?.oneMinuteIncrease == null ? "수집 중" : `${item.trend.oneMinuteIncrease >= 0 ? "+" : ""}${item.trend.oneMinuteIncrease.toFixed(2)}%p`}</td><td>{formatKoreanAmount(item.marketCap)}</td><td>{formatKoreanAmount(item.tradingValue)}</td></tr>; })}</tbody></table></div>}
    </div>
  </main>;
}
