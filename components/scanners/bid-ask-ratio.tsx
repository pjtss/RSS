...
import type { BidAskRatioItem } from "@/lib/kis";
import styles from "./scanner.module.css";
import { GLOBAL_POLLING_INTERVAL } from "@/lib/constants";

export function BidAskRatio() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/api/stock/bid-ask-ratio", { cache: "no-store" });
          ...
          {items.slice(0, 5).map((item) => (
            <div key={item.code} className={styles.item}>
              ...
              <div className={styles.info}>
                ...
                <span className={styles.highlight}>매수잔량 {item.bidAskRatio}% 우위</span>
              </div>
              ...
              <div className={styles.barWrap}>
                <div 
                  ...
                  style={{ width: `${Math.min(100, item.bidAskRatio / 3)}%` }} 
                />
              </div>
            </div>
          ))}
