...
import type { NewHighItem } from "@/lib/kis";
import styles from "./scanner.module.css";
import { GLOBAL_POLLING_INTERVAL } from "@/lib/constants";

export function NewHigh() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/api/stock/new-high", { cache: "no-store" });
          ...
          {items.slice(0, 5).map((item) => (
            <div key={item.code} className={styles.item}>
              ...
              <div className={styles.info}>
                ...
                <span className={styles.highlight}>{item.highType} 돌파</span>
              </div>
