...
import type { NewHighItem } from "@/lib/kis-us";
import styles from "../scanner.module.css";
import { GLOBAL_POLLING_INTERVAL } from "@/lib/constants";

export function UsNewHigh() {
  const [items, setItems] = useState<NewHighItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/api/stock/us/new-high", { cache: "no-store" });
          ...
          {items.slice(0, 5).map((item) => (
            <div key={item.code} className={styles.item}>
              ...
              <div className={styles.info}>
                ...
                <span className={styles.highlightBadge}>{item.highType}</span>
              </div>
