...
import type { VolumeSpikeItem } from "@/lib/kis";
import styles from "./scanner.module.css";
import { GLOBAL_POLLING_INTERVAL } from "@/lib/constants";

export function VolumeSpike() {
  const [items, setItems] = useState<VolumeSpikeItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/api/stock/volume-spike", { cache: "no-store" });
          ...
          {items.slice(0, 5).map((item) => (
            <div key={item.code} className={styles.item}>
              ...
              <div className={styles.info}>
                ...
                <span className={styles.highlight}>대금 {item.tradingValue} (전일비 {item.volumeRatio})</span>
              </div>
