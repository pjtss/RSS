...
## 2026-05-18
- **[기능 개선]** 로컬/개발기 KIS 자격증명 부재 하에서도 원격 Neon DB 캐시 자동 연동 고도화
  - **자격증명 누락 시 DB 복원 연동**: 로컬 개발 환경(`.env.local`)에 `KIS_APPKEY`가 정의되어 있지 않더라도, `DATABASE_URL`이 매핑되어 있는 상태라면 Neon DB의 `kis_cache` 테이블에서 실서버가 크론/정규장에 기록해둔 최신 실거래 종가 데이터를 가져오도록 복원 파이프라인 업그레이드.
