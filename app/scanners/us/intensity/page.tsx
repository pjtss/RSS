...
export default function UsTradingIntensityScannerPage() {
  const [stocks, setStocks] = useState<UsIntensityStockItem[]>([]);
  ...
  const fetchStocks = async (isAuto = false) => {
    try {
      ...
      setError(null);
      const res = await fetch("/api/stock/us/intensity");
      ...
      if (debugStatus === "empty" || debugStatus === "error") {
        console.warn(`🚨 [KIS-DEBUG-CLIENT] 미국 체결강도 TOP 10 데이터 없음 감지! 상태: [${debugStatus}], 원인: ${debugReason}`);
        setIsFallback(false);
      } else if (debugStatus === "fallback") {
        console.warn(`⚠️ [KIS-DEBUG-CLIENT] 미국 체결강도 KIS API 장애에 따른 DB 캐시 폴백 감지! 원인: ${debugReason}`);
        setIsFallback(true);
      } else {
        console.info(`⚡ [KIS-DEBUG-CLIENT] 미국 체결강도 TOP 10 데이터 로드 성공: ${debugReason}`);
        setIsFallback(false);
      }
  ...
  const handleAutoSync = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        console.info("⚡ [KIS-DEBUG-CLIENT] 미국 체결강도 자동 동기화 응답:", data);
        if (data.success) {
          await fetchStocks(true);
        }
      }
    } catch (err) {
      console.warn("Failed auto-syncing us trading intensity:", err);
    }
  };
  ...
  return (
    <main className={styles.page}>
      <PageNavigation current="us-intensity" />
